...
void OnKillEnemy(Player * pPlayer, Player * pKilled)
{
	...
	if( kills >= 15 && ( !pPlayer->HasKnownTitle( PVPTITLE_SCOUT ) && !pPlayer->HasKnownTitle( PVPTITLE_PRIVATE ) ) )
	{
		pPlayer->SetKnownTitle( pPlayer->GetTeam() ? PVPTITLE_SCOUT : PVPTITLE_PRIVATE, true  );
	...
	if( kills >= 150 && !pPlayer->HasKnownTitle( PVPTITLE_GRUNT ) && !pPlayer->HasKnownTitle( PVPTITLE_CORPORAL ) )
	{
	...
	if( kills >= 300 && !pPlayer->HasKnownTitle( PVPTITLE_SERGEANT ) && !pPlayer->HasKnownTitle( PVPTITLE_HSERGEANT ) )
	{
		pPlayer->SetKnownTitle( pPlayer->GetTeam() ? PVPTITLE_SERGEANT : PVPTITLE_HSERGEANT, true  );
		pPlayer->BroadcastMessage( "You have earned new title!" );
	};

	if( kills >= 800 && !pPlayer->HasKnownTitle( PVPTITLE_SENIOR_SERGEANT ) && !pPlayer->HasKnownTitle( PVPTITLE_MASTER_SERGEANT ) )
	{
		pPlayer->SetKnownTitle( pPlayer->GetTeam() ? PVPTITLE_SENIOR_SERGEANT : PVPTITLE_MASTER_SERGEANT, true  );
		pPlayer->BroadcastMessage( "You have earned new title!" );
	};

	if( kills >= 1400 && !pPlayer->HasKnownTitle( PVPTITLE_FIRST_SERGEANT ) && !pPlayer->HasKnownTitle( PVPTITLE_SERGEANT_MAJOR ) )
	{
		pPlayer->SetKnownTitle( pPlayer->GetTeam() ? PVPTITLE_FIRST_SERGEANT : PVPTITLE_SERGEANT_MAJOR , true  );
		pPlayer->BroadcastMessage( "You have earned new title!" );
	};

	if( kills >= 1800 && !pPlayer->HasKnownTitle( PVPTITLE_KNIGHT ) && !pPlayer->HasKnownTitle( PVPTITLE_STONE_GUARD ) )
	{
		pPlayer->SetKnownTitle( pPlayer->GetTeam() ? PVPTITLE_STONE_GUARD : PVPTITLE_KNIGHT, true  );
		pPlayer->BroadcastMessage( "You have earned new title!" );
	};

	if( kills >= 2200 && !pPlayer->HasKnownTitle( PVPTITLE_KNIGHT_LIEUTENANT ) && !pPlayer->HasKnownTitle( PVPTITLE_BLOOD_GUARD ) )
	{
		pPlayer->SetKnownTitle( pPlayer->GetTeam() ? PVPTITLE_BLOOD_GUARD : PVPTITLE_KNIGHT_LIEUTENANT, true  );
		pPlayer->BroadcastMessage( "You have earned new title!" );
	};

	if( kills >= 2800 && !pPlayer->HasKnownTitle( PVPTITLE_KNIGHT_CAPTAIN ) && !pPlayer->HasKnownTitle( PVPTITLE_LEGIONNAIRE ) )
	{
		pPlayer->SetKnownTitle( pPlayer->GetTeam() ? PVPTITLE_LEGIONNAIRE : PVPTITLE_KNIGHT_CAPTAIN, true  );
		pPlayer->BroadcastMessage( "You have earned new title!" );
	};

	if( kills >= 3400 && !pPlayer->HasKnownTitle( PVPTITLE_KNIGHT_CHAMPION ) && !pPlayer->HasKnownTitle( PVPTITLE_CENTURION ) )
	{
		pPlayer->SetKnownTitle( pPlayer->GetTeam() ? PVPTITLE_CENTURION : PVPTITLE_KNIGHT_CHAMPION, true  );
		pPlayer->BroadcastMessage( "You have earned new title!" );
	};

	if( kills >= 4000 && !pPlayer->HasKnownTitle( PVPTITLE_LIEUTENANT_COMMANDER ) && !pPlayer->HasKnownTitle( PVPTITLE_CHAMPION ) )
	{
		pPlayer->SetKnownTitle( pPlayer->GetTeam() ? PVPTITLE_CHAMPION : PVPTITLE_LIEUTENANT_COMMANDER, true  );
		pPlayer->BroadcastMessage( "You have earned new title!" );
	};

	if( kills >= 4500 && !pPlayer->HasKnownTitle( PVPTITLE_COMMANDER ) && !pPlayer->HasKnownTitle( PVPTITLE_LIEUTENANT_GENERAL ) )
	{
		pPlayer->SetKnownTitle( pPlayer->GetTeam() ? PVPTITLE_LIEUTENANT_GENERAL : PVPTITLE_COMMANDER, true  );
		pPlayer->BroadcastMessage( "You have earned new title!" );
	};

	if( kills >= 5200 && !pPlayer->HasKnownTitle( PVPTITLE_MARSHAL ) && !pPlayer->HasKnownTitle( PVPTITLE_GENERAL ) )
	{
		pPlayer->SetKnownTitle( pPlayer->GetTeam() ? PVPTITLE_GENERAL : PVPTITLE_MARSHAL, true  );
		pPlayer->BroadcastMessage( "You have earned new title!" );
	};

	if( kills >= 6000 && !pPlayer->HasKnownTitle( PVPTITLE_FIELD_MARSHAL ) && !pPlayer->HasKnownTitle( PVPTITLE_WARLORD ) )
	{
		pPlayer->SetKnownTitle( pPlayer->GetTeam() ? PVPTITLE_WARLORD : PVPTITLE_FIELD_MARSHAL, true  );
		pPlayer->BroadcastMessage( "You have earned new title!" );
	};

	if( kills >= 7000 && !pPlayer->HasKnownTitle( PVPTITLE_GRAND_MARSHAL ) && !pPlayer->HasKnownTitle( PVPTITLE_WARLORD ) )
	{
		pPlayer->SetKnownTitle( pPlayer->GetTeam() ? PVPTITLE_HIGH_WARLORD : PVPTITLE_GRAND_MARSHAL, true  );
		pPlayer->BroadcastMessage( "You have earned new title!" );
	};

	if( kills >= 10000 && !pPlayer->HasKnownTitle( PVPTITLE_CONQUEROR ) )
	{
