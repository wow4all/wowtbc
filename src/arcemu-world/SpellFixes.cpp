...
//	printf("!!!!!!! name %s , id %u , hash %u \n",sp->Name,sp->Id, namehash);
	...
	if(sp != NULL)
	{
		...
		//Arcane Orb
	    sp = dbcSpell.LookupEntryForced(34172);
	    if(sp != NULL)
	    {
		sp->EffectRadiusIndex[0] = 28;
		...
		//Aviana's Will - proc interval 10s
	   ...
	   sp = dbcSpell.LookupEntryForced(37742); 
	   if(sp != NULL)
	  {
	  ...
	  // Love Struck
	  sp = dbcSpell.LookupEntryForced(34584);
	  if(sp != NULL)
	 {
		sp->procFlags = PROC_ON_CAST_SPELL;
	 }
	 ...
	 //Pyroblast - Kael'thas
	 sp = dbcSpell.LookupEntryForced(36819);
	 if(sp != NULL)
	{
	...
	//Divine Favor - removed from the spell hit goal
	...
	//Shatter - Gruul
	sp = dbcSpell.LookupEntryForced(33671);
	if(sp != NULL)
	{
		sp->EffectImplicitTargetA[0] = EFF_TARGET_NONE;
	}
	
	//Rain of Bones - Nightbane
	sp = dbcSpell.LookupEntryForced(37098);
	if(sp != NULL)
	{
	...
	sp = dbcSpell.LookupEntryForced(45484);
	if(sp != NULL)
	{
		sp->proc_interval = 45000;
	}
	...
	sp = dbcSpell.LookupEntryForced(45483);
	if(sp != NULL)
	{
		sp->proc_interval = 45000;
	}
	...
	sp = dbcSpell.LookupEntryForced(45481);
	if(sp != NULL)
	{
		sp->proc_interval = 45000;
	}
	...
	sp = dbcSpell.LookupEntryForced(45482);
	if(sp != NULL)
	{
		sp->proc_interval = 45000;
	}
	
	//Fear Ward - little special
	sp = dbcSpell.LookupEntryForced(6346);
	if(sp != NULL)
	{
		sp->procFlags = PROC_ON_RESIST_VICTIM;
	}
	...
	//Brutal Libram of Justice - proc range
	sp = dbcSpell.LookupEntryForced(46093);
	if(sp != NULL)
	{
		sp->rangeIndex = 6;
	}
	...
	//Unquenchable Flames
	sp = dbcSpell.LookupEntryForced(31341);
	if( sp != NULL)
	{
	...
	//Inferno (Anetheron)
	sp = dbcSpell.LookupEntryForced(31299);
	if( sp != NULL)
	{
		sp->EffectImplicitTargetA[0] = EFF_TARGET_SINGLE_ENEMY;
	}
	...
	//Vampiric Aura
	sp = dbcSpell.LookupEntryForced(38196);
	if( sp != NULL )
	{
	...
	//Sleep - single target
	sp = dbcSpell.LookupEntryForced(31298);
	if(sp != NULL)
	{
		sp->EffectImplicitTargetA[0] = EFF_TARGET_SINGLE_ENEMY;
		sp->EffectImplicitTargetB[0] = EFF_TARGET_SINGLE_ENEMY;
	...
	//Mark of Kaz'rogal
	sp = dbcSpell.LookupEntryForced(31463);
	if(sp != NULL)
	{
		sp->EffectImplicitTargetA[0] = EFF_TARGET_SELF;
	}
	...
	if(sp != NULL)
	{
		sp->procFlags = PROC_ON_SPELL_HIT | PROC_ON_MELEE_ATTACK;

		//http://www.wowhead.com/?spell=46578#comments:id=308744
		//has a better explanation
		sp->proc_interval = 20000;
	}
	
	//Pagle's Point Cast - Create Mudskunk Lure
	sp = dbcSpell.LookupEntryForced(24325);
	if(sp != NULL)
	{
		sp->EffectImplicitTargetA[0] = EFF_TARGET_SELF;
	}
	...
	//Static Charge
	sp = dbcSpell.LookupEntryForced(38281);
	if(sp != NULL)
	{
	...
	sp = dbcSpell.LookupEntryForced(43736);
	if(sp != NULL)
	{
	...
	SpellEntry * sp_healing_wave = dbcSpell.LookupEntryForced(8004);	//range  healing wave
	...
	//Molten Flame
	sp = dbcSpell.LookupEntryForced(40980);
	if(sp != NULL)
	{
		sp->EffectImplicitTargetA[0] = EFF_TARGET_SINGLE_ENEMY;
	}
	sp = dbcSpell.LookupEntryForced(40253);
	if(sp != NULL)
	{
		sp->EffectImplicitTargetA[0] = EFF_TARGET_ALL_FRIENDLY_IN_AREA;
		sp->EffectImplicitTargetA[1] = EFF_TARGET_ALL_FRIENDLY_IN_AREA;
		sp->EffectImplicitTargetA[2] = EFF_TARGET_ALL_FRIENDLY_IN_AREA;		
		sp->EffectImplicitTargetB[0] = EFF_TARGET_ALL_FRIENDLY_IN_AREA;
		sp->EffectImplicitTargetB[1] = EFF_TARGET_ALL_FRIENDLY_IN_AREA;
		sp->EffectImplicitTargetB[2] = EFF_TARGET_ALL_FRIENDLY_IN_AREA;
	}
	
	//Watery Grave Explosion
	sp = dbcSpell.LookupEntryForced(37852);
	if(sp != NULL)
	{
		sp->EffectImplicitTargetA[0] = EFF_TARGET_SINGLE_ENEMY;
		...
		sp->EffectImplicitTargetB[0] = EFF_TARGET_SINGLE_ENEMY;
		sp->EffectImplicitTargetB[1] = EFF_TARGET_SINGLE_ENEMY;
		sp->EffectImplicitTargetB[2] = EFF_TARGET_SINGLE_ENEMY;
	}
	
	//Watery Grave
	sp = dbcSpell.LookupEntryForced(38049);
	if(sp != NULL)
	{
		sp->EffectImplicitTargetA[0] = EFF_TARGET_SINGLE_ENEMY;
		sp->EffectImplicitTargetA[1] = EFF_TARGET_SINGLE_ENEMY;
	...
	//Ground Slam
	sp = dbcSpell.LookupEntryForced(39187);
	if(sp != NULL)
	{
		sp->EffectImplicitTargetA[0] = EFF_TARGET_SINGLE_ENEMY;
	}
	//Gronn Lord's Grasp
	sp = dbcSpell.LookupEntryForced(33572);
	if(sp != NULL)
	{
		sp->EffectImplicitTargetA[0] = EFF_TARGET_SINGLE_ENEMY;
	}

	//Spell Haste Trinket - proc rate 45s
	...
	//Arena preparation
	sp = dbcSpell.LookupEntryForced(32727);
	if(sp != NULL)
	{
	...
	//Blessing of Righteousness - proc (do not have rewrite blessing)
	sp = dbcSpell.LookupEntryForced(37198);
	if(sp != NULL)
		sp->BGR_one_buff_on_target = 0;

	//blessing of why theories
	sp = dbcSpell.LookupEntryForced(35076);
	if(sp != NULL)
		sp->BGR_one_buff_on_target = 0;
	sp = dbcSpell.LookupEntryForced(5862);
	if(sp != NULL)
		sp->BGR_one_buff_on_target = 0;
	sp = dbcSpell.LookupEntryForced(33377);
	if(sp != NULL)
		sp->BGR_one_buff_on_target = 0;
	sp = dbcSpell.LookupEntryForced(8733);
	if(sp != NULL)
		sp->BGR_one_buff_on_target = 0;
	sp = dbcSpell.LookupEntryForced(40452);
	if(sp != NULL)
		sp->BGR_one_buff_on_target = 0;
	sp = dbcSpell.LookupEntryForced(46119);
	if(sp != NULL)
		sp->BGR_one_buff_on_target = 0;
	sp = dbcSpell.LookupEntryForced(40446);
	if(sp != NULL)
		sp->BGR_one_buff_on_target = 0;
	sp = dbcSpell.LookupEntryForced(37877);
	if(sp != NULL)
		sp->BGR_one_buff_on_target = 0;
	sp = dbcSpell.LookupEntryForced(38332);
	if(sp != NULL)
		sp->BGR_one_buff_on_target = 0;
	sp = dbcSpell.LookupEntryForced(37878);
	if(sp != NULL)
		sp->BGR_one_buff_on_target = 0;
	sp = dbcSpell.LookupEntryForced(37879);
	if(sp != NULL)
		sp->BGR_one_buff_on_target = 0;
	sp = dbcSpell.LookupEntryForced(37880);
	if(sp != NULL)
		sp->BGR_one_buff_on_target = 0;
	sp = dbcSpell.LookupEntryForced(37881);
	if(sp != NULL)
		sp->BGR_one_buff_on_target = 0;
	sp = dbcSpell.LookupEntryForced(41250);
	if(sp != NULL)
		sp->BGR_one_buff_on_target = 0;
	sp = dbcSpell.LookupEntryForced(23108);
	if(sp != NULL)
		sp->BGR_one_buff_on_target = 0;
	sp = dbcSpell.LookupEntryForced(39542);
	if(sp != NULL)
		sp->BGR_one_buff_on_target = 0;
	sp = dbcSpell.LookupEntryForced(39826);
	if(sp != NULL)
		sp->BGR_one_buff_on_target = 0;
	sp = dbcSpell.LookupEntryForced(40445);
	if(sp != NULL)
		sp->BGR_one_buff_on_target = 0;
	sp = dbcSpell.LookupEntryForced(16599);
	if(sp != NULL)
		sp->BGR_one_buff_on_target = 0;
	sp = dbcSpell.LookupEntryForced(41450);
	if(sp != NULL)
		sp->BGR_one_buff_on_target = 0;
	sp = dbcSpell.LookupEntryForced(41451);
	if(sp != NULL)
		sp->BGR_one_buff_on_target = 0;
	sp = dbcSpell.LookupEntryForced(23720);
	if(sp != NULL)
		sp->BGR_one_buff_on_target = 0;
	sp = dbcSpell.LookupEntryForced(28750);
	if(sp != NULL)
		sp->BGR_one_buff_on_target = 0;
	sp = dbcSpell.LookupEntryForced(60597);
	if(sp != NULL)
		sp->BGR_one_buff_on_target = 0;
	sp = dbcSpell.LookupEntryForced(58026);
	if(sp != NULL)
		sp->BGR_one_buff_on_target = 0;
	sp = dbcSpell.LookupEntryForced(3591);
	if(sp != NULL)
		sp->BGR_one_buff_on_target = 0;
	sp = dbcSpell.LookupEntryForced(48183);
	if(sp != NULL)
		sp->BGR_one_buff_on_target = 0;
	sp = dbcSpell.LookupEntryForced(54323);
	if(sp != NULL)
		sp->BGR_one_buff_on_target = 0;
	sp = dbcSpell.LookupEntryForced(35163);
	if(sp != NULL)
		sp->BGR_one_buff_on_target = 0;
	sp = dbcSpell.LookupEntryForced(51442);
	if(sp != NULL)
		sp->BGR_one_buff_on_target = 0;
	sp = dbcSpell.LookupEntryForced(28449);
	if(sp != NULL)
		sp->BGR_one_buff_on_target = 0;
	...
	/**************END BLESSING******************/
		...
		//Sanctified Seals rank 2
	sp = dbcSpell.LookupEntryForced(35396);
	if(sp != NULL)
	{
		sp->EffectSpellGroupRelation[1] = 512 | 4096 | 262144 | 33554432 | 134217728;
		sp->EffectSpellGroupRelation_high[1] = 1024;
	}
	
	//Sanctified Seals rank 3
	sp = dbcSpell.LookupEntryForced(35397);
	if(sp != NULL)
	{
		sp->EffectSpellGroupRelation[1] = 512 | 4096 | 262144 | 33554432 | 134217728;
		sp->EffectSpellGroupRelation_high[1] = 1024;
	...
	sp = dbcSpell.LookupEntryForced(38499);
	if(sp != NULL)
	{
		sp->EffectSpellGroupRelation_high[0] = 128;
	}

	//Nature's Guardian - rank 1
	...
	if(sp != NULL)
	{
		sp->EffectSpellGroupRelation[0] = 1024 | 524288 | 1048576 | 8388608 | 536870912;
		sp->EffectSpellGroupRelation_high[0] = 1 | 8 | 520;
	}
	...
	//Sanctified Seals rank 1
	sp = dbcSpell.LookupEntryForced(32043);
	if(sp != NULL)
	{
		sp->EffectSpellGroupRelation[1] = 512 | 4096 | 262144 | 33554432 | 134217728;
		sp->EffectSpellGroupRelation_high[1] = 1024;
	...
	sp = dbcSpell.LookupEntryForced(42787);
	if(sp != NULL)
	{
		sp->EffectImplicitTargetA[0] = EFF_TARGET_ALL_FRIENDLY_IN_AREA;
		sp->EffectImplicitTargetA[1] = EFF_TARGET_ALL_FRIENDLY_IN_AREA;
	...
	sp = dbcSpell.LookupEntryForced(42783);
	if(sp != NULL)
	{
		sp->EffectAmplitude[0] = 6000;
		sp->EffectTriggerSpell[0] = 42787;
		sp->always_apply = true;
	}
	
    //Seal of Command - proc rate 
	//rank 1
	...
	//Fel Siphon
	sp = dbcSpell.LookupEntryForced(44937);
	if(sp != NULL)
		sp->Effect[0] = SPELL_EFFECT_DUMMY;

	//Fel Zapper
	sp = dbcSpell.LookupEntryForced(35282);
	if(sp != NULL)
		sp->Effect[1] = SPELL_EFFECT_NULL;
	...
	if(sp != NULL)
	{
		sp->EffectApplyAuraName[0] = SPELL_AURA_MOD_SPELL_CRIT_CHANCE_SCHOOL;
	}
	
	//Prayer of Healing - 
	sp = dbcSpell.LookupEntryForced(33152);
    if(sp != NULL)
	{
		sp->EffectImplicitTargetA[0] = EFF_TARGET_ALL_FRIENDLY_IN_AREA;
	...
	sp = dbcSpell.LookupEntryForced(29528);
    if(sp != NULL)
		sp->Effect[1] = SPELL_EFFECT_DUMMY;

	//Flame Strike
	sp = dbcSpell.LookupEntryForced(46163);
    if(sp != NULL)
	{
		sp->EffectBasePoints[0] = 3125; //zmensime dmg
	}
	
	//Enfeeble - single target
	sp = dbcSpell.LookupEntryForced(30843);
    if(sp != NULL)
	{
		sp->EffectImplicitTargetA[0] = EFF_TARGET_SINGLE_ENEMY;
		sp->EffectImplicitTargetA[1] = EFF_TARGET_SINGLE_ENEMY;
	...
	//http://www.wowhead.com/?spell=37566
	sp = dbcSpell.LookupEntryForced(37566);
    if(sp != NULL)
	{
		sp->EffectImplicitTargetA[0] = EFF_TARGET_ALL_FRIENDLY_IN_AREA;
		sp->EffectImplicitTargetA[1] = EFF_TARGET_ALL_FRIENDLY_IN_AREA;
	...
	//http://www.wowhead.com/?spell=30695
	sp = dbcSpell.LookupEntryForced(30695);
    if(sp != NULL)
	{
		sp->EffectImplicitTargetA[0] = EFF_TARGET_ALL_FRIENDLY_IN_AREA;
		sp->EffectImplicitTargetA[1] = EFF_TARGET_ALL_FRIENDLY_IN_AREA;
	...
	//Cozy Fire - buff
	sp = dbcSpell.LookupEntryForced(7353);
	if(sp != NULL)
	sp->c_is_flags |= SPELL_FLAG_IS_FORCEDBUFF;
	sp = dbcSpell.LookupEntryForced(7358);
	if(sp != NULL)
	sp->c_is_flags |= SPELL_FLAG_IS_FORCEDBUFF;
	
	//Garrote - Morroes everytime
	sp = dbcSpell.LookupEntryForced(37066);
	if(sp != NULL)
	...
	// Swift Flying Broom + all sorts of brooms
		...
		sp = dbcSpell.LookupEntryForced(42667);
		if(sp != NULL && sp2 != NULL)
		{
			sp->CastingTimeIndex = sp2->CastingTimeIndex; // Set cast time 3 sec
		}
		...
		sp = dbcSpell.LookupEntryForced(42680);
		if(sp != NULL && sp2 != NULL)
		{
			sp->CastingTimeIndex = sp2->CastingTimeIndex; // Set cast time 3 sec
		}
		sp = dbcSpell.LookupEntryForced(48024);
		if(sp != NULL && sp2 != NULL)
		{
			sp->CastingTimeIndex = sp2->CastingTimeIndex; // Set cast time 3 sec
		}
		...
		sp = dbcSpell.LookupEntryForced (5785);
		if(sp != NULL)
		{
			sp->Effect[1] = SPELL_EFFECT_LEARN_SPELL;
			sp->EffectTriggerSpell[1] = 33388;		
		}
		 ///////////////////////////
		//Boss spell fixes here////
		///////////////////////////
		
		//Mother Shahraz's Saber Lash
	    //Claw Rage - Zul'jin
	    sp = dbcSpell.LookupEntryForced(43150);
	    sp->EffectImplicitTargetA[0] = EFF_TARGET_SCRIPTED_OR_SINGLE_TARGET;
		
		//Surge - Nalorakk
	    sp = dbcSpell.LookupEntryForced(42402);
	    sp->EffectImplicitTargetA[1] = EFF_TARGET_ALL_ENEMY_IN_AREA;
	    sp->EffectImplicitTargetA[2] = EFF_TARGET_ALL_ENEMY_IN_AREA;
		
		//Static disruption
	    sp = dbcSpell.LookupEntryForced(43622);
		...
		//Akil'zon Electrical Storm
	sp = dbcSpell.LookupEntryForced(43657);
	if(sp != NULL)
	{
		sp->EffectImplicitTargetA[0] = EFF_TARGET_NONE;
		sp->EffectImplicitTargetB[0] = EFF_TARGET_SCRIPTED_OR_SINGLE_TARGET;
		sp->Effect[0] = SPELL_EFFECT_DUMMY;
	}
	sp = dbcSpell.LookupEntryForced(43648);
	sp->EffectTriggerSpell[1] = 43657;
	
	//Cyclone - Zul'jin add
	sp = dbcSpell.LookupEntryForced(43121);
	if(sp != NULL)
	{
		sp->rangeIndex = 2;
	}
	sp = dbcSpell.LookupEntryForced(43119);
	sp->EffectApplyAuraName[0] = SPELL_AURA_PERIODIC_TRIGGER_SPELL;
	sp->EffectAmplitude[0] = 1000;
	sp->EffectTriggerSpell[0] = 43121;
		...
		//Dragonmaw Illusion
	    sp = dbcSpell.LookupEntryForced(40216);
	    sp->EffectImplicitTargetA[1] = 1;
	    sp->Effect[1] = SPELL_EFFECT_APPLY_AURA;
	    sp->EffectApplyAuraName[1] = SPELL_AURA_TRANSFORM;
	    sp->EffectMiscValue[1] = 23130;
	...
	////////////////////////////////////	
	////More class spell fixes here/////
	////////////////////////////////////
		
    //Shaman - Frostband Weapon - 8PPM
	sp = dbcSpell.LookupEntryForced(8034);
	sp->proc_interval = 7500;
	sp->fixed_dddhcoef = 0;
	//rank 2
	sp = dbcSpell.LookupEntryForced(8037);
	sp->proc_interval = 7500;
	...
	sp->EffectSpellGroupRelation[0] = 356;
	sp->EffectSpellGroupRelation_high[0] = 128;
	...
	//Energy Storm
	sp = dbcSpell.LookupEntryForced(43983);
	sp->EffectImplicitTargetA[0] = EFF_TARGET_ALL_ENEMIES_AROUND_CASTER;
	
	//Druid lifebloom bonuses
	//S4
	sp = dbcSpell.LookupEntryForced(46100);
	sp->EffectSpellGroupRelation_high[0] = 16;
	//T5
	sp = dbcSpell.LookupEntryForced(37314);
	sp->EffectSpellGroupRelation_high[0] = 16;
	//S3
	sp = dbcSpell.LookupEntryForced(43725);
	sp->EffectSpellGroupRelation_high[0] = 16;
	//S2
	sp = dbcSpell.LookupEntryForced(34128);
	sp->EffectSpellGroupRelation_high[0] = 16;
	//S1
	sp = dbcSpell.LookupEntryForced(42367);
	sp->EffectSpellGroupRelation_high[0] = 16;
	    ...
	    sp = dbcSpell.LookupEntryForced(37522);
	    if(sp != NULL)
	    {
		sp->procFlags = PROC_ON_CAST_SPELL;
...
//			sp->procFlags = PROC_ON_TARGET_DIE;
		...
		//Psychic Scream
		sp = dbcSpell.LookupEntryForced(8122); //rank 1
		if(sp != NULL)
			sp->base_range_or_radius_sqr = 8.0f;

		sp = dbcSpell.LookupEntryForced(8124); //rank 2
		if(sp != NULL)
			sp->base_range_or_radius_sqr = 8.0f;

		sp = dbcSpell.LookupEntryForced(10888); //rank 3
		if(sp != NULL)
			sp->base_range_or_radius_sqr = 8.0f;

		sp = dbcSpell.LookupEntryForced(10890); //rank 4
		if(sp != NULL)
			sp->base_range_or_radius_sqr = 8.0f;
	...
	// Insert shaman spell fixes here
        ...
        //Healing Way
        sp = dbcSpell.LookupEntryForced(29206);		//rank 1
        if( sp != NULL )
            sp->EffectSpellGroupRelation[0] = 64;
        sp = dbcSpell.LookupEntryForced(29205);		//rank 2
        if( sp != NULL )
            sp->EffectSpellGroupRelation[0] = 64;
        sp = dbcSpell.LookupEntryForced(29202);		//rank 3
        if( sp != NULL )
            sp->EffectSpellGroupRelation[0] = 64;
	...
	// Insert warlock spell fixes here
		...
		if( sp != NULL )
		{
            ...
            sp->ProcOnNameHash[1] = SPELL_HASH_CORRUPTION;
			sp->procChance = 2;
		}
	...
	// Insert items spell fixes here
		...
		//if( sp != NULL )
		//{
			//sp->procFlags = CUSTOMPROC_ON_ADDCOMBO; <Keep getting undeclared identifier here ? ;/ even tho the flag is in spell.h 
        ...
        //Mongoose
        sp = dbcSpell.LookupEntryForced( 28093 );
        if( sp != NULL )
            sp->maxstack = 2;
