		...
		else
		{
			...
			for(set<Player*>::iterator itr = contributors.begin(); itr != contributors.end(); itr++)
			{
				...
				if(pAffectedPlayer->GetZoneId() == 3483)
				{
					...
					if(pAffectedPlayer->GetMapMgr()->GetWorldStateManager()->GetWorldState(WORLDSTATE_HELLFIRE_HORDE_TOWERS_CONTROLLED) != 3 && pAffectedPlayer->GetTeam() == 1) 
 						return;

					// Hellfire Alliance Controlled Towers
					if(pAffectedPlayer->GetMapMgr()->GetWorldStateManager()->GetWorldState(WORLDSTATE_HELLFIRE_ALLIANCE_TOWERS_CONTROLLED) != 3 && pAffectedPlayer->GetTeam() == 0) 
						return;
