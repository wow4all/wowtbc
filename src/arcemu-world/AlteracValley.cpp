...
static const AVSpawnLocation g_fireLocations[AV_NUM_CONTROL_POINTS][30] = {
...
static const uint32 g_unitWaypoints[4][2] = {
	{  },
};
...
static const AVSpawnLocation g_initalGuardLocations[AV_NUM_CONTROL_POINTS][30] = {
...
static const AVHomeNPC g_HomeNpcInfo[AV_NUM_CONTROL_POINTS] = {
...
static const AVNodeTemplate g_nodeTemplates[AV_NUM_CONTROL_POINTS] = {
...
static const uint32 g_gameObjectFactions[AV_NODE_STATE_COUNT] = {
	35,				// neutral
	2,				// alliance assault
...
static const int32 g_stateToGuardType[AV_NODE_STATE_COUNT] = {
	0,				// neutral
	-1,				// alliance assault
...
AVNode::AVNode(AlteracValley &parent, AVNodeTemplate const *tmpl, uint32 nodeid) : m_bg(parent), m_template(tmpl), m_nodeId(nodeid)
{
	m_boss = NULL;
	m_flag = NULL;
	m_aura = NULL;
	...
	if(g_HomeNpcInfo[m_nodeId].id_a != 0)
	{
		...
		{
			m_homeNPC = m_bg.GetMapMgr()->GetInterface()->SpawnCreature(g_HomeNpcInfo[m_nodeId].id_a, g_HomeNpcInfo[m_nodeId].a_x, g_HomeNpcInfo[m_nodeId].a_y, g_HomeNpcInfo[m_nodeId].a_z, g_HomeNpcInfo[m_nodeId].a_o, true, false, 0, 0);
		}
		else
		{
			m_homeNPC = m_bg.GetMapMgr()->GetInterface()->SpawnCreature(g_HomeNpcInfo[m_nodeId].id_h, g_HomeNpcInfo[m_nodeId].h_x, g_HomeNpcInfo[m_nodeId].h_y, g_HomeNpcInfo[m_nodeId].h_z, g_HomeNpcInfo[m_nodeId].h_o, true, false, 0, 0);
		}
	}
}
...
AVNode::~AVNode()
{
	for(vector<Creature*>::const_iterator itr = m_guards.begin();itr != m_guards.end();++itr)
	{
		(*itr)->Despawn(0, 0);
	}

	for(vector<Creature*>::const_iterator itr = m_peonLocations.begin();itr != m_peonLocations.end();++itr)
	{
		(*itr)->Despawn(0, 0);
	}
	...
	if(m_flag)
	{
		m_flag->m_battleground = NULL;
		delete m_flag;
	}

	if(m_aura)
	{
		m_aura->m_battleground = NULL;
		delete m_aura;
	}

	if(m_glow)
	{
		m_glow->m_battleground = NULL;
		delete m_glow;
	}
...
void AVNode::Assault(Player *plr)
{
	...
	// safety check
	if(plr->GetTeam() == ALLIANCE && (m_state == AV_NODE_STATE_ALLIANCE_ASSAULTING || m_state == AV_NODE_STATE_ALLIANCE_CONTROLLED))
		return;

	if(plr->GetTeam() == HORDE && (m_state == AV_NODE_STATE_HORDE_ASSAULTING || m_state == AV_NODE_STATE_HORDE_CONTROLLED))
		return;
	...
	if ((plr->GetTeam() == ALLIANCE && m_lastState == AV_NODE_STATE_ALLIANCE_CONTROLLED) ||
		(plr->GetTeam() == HORDE && m_lastState == AV_NODE_STATE_HORDE_CONTROLLED))
	{
		// set the state for capture
		m_state = (plr->GetTeam() == ALLIANCE) ? AV_NODE_STATE_ALLIANCE_ASSAULTING : AV_NODE_STATE_HORDE_ASSAULTING;
...
void AVNode::Spawn()
{
	...
	else
	{
		...
		if(m_flag == NULL)
		{
			...
			m_flag->AddToWorld(m_bg.GetMapMgr());
		}
		else
		{
	...
	else
	{
		...
		if(m_aura == NULL)
		{
			...
			m_aura->AddToWorld(m_bg.GetMapMgr());
		}
		else
		{
			// change entry, but to do this change guid
	...
	else
	{
		...
		if(m_glow == NULL)
		{
			...
			m_glow->AddToWorld(m_bg.GetMapMgr());
		}
		else
		{
			// change entry, but to do this change guid
	...
	if(m_state == AV_NODE_STATE_ALLIANCE_CONTROLLED || m_state == AV_NODE_STATE_HORDE_CONTROLLED)
	{
		...
		if(t > 0 && m_template->m_guardId[t] != 0)
		{
			for(uint32 i = 0; i < m_template->m_guardCount; ++i)
			{
				...
				if(z != 0.0f)
				{
					m_guards.push_back(m_bg.GetMapMgr()->GetInterface()->SpawnCreature(m_template->m_guardId[t], x, y, z, 0.0f, true, false, 0, 0));
				}
			}
		}
	}
...
void AVNode::Capture()
{
	...
	if(!m_template->m_capturable)
	{
		if(m_state != m_template->m_defaultState)
		{
			...
			if(m_template->m_defaultState == AV_NODE_STATE_ALLIANCE_CONTROLLED)
			{
				for(PlayerSet::iterator itx = m_bg.m_players[1].begin(); itx != m_bg.m_players[1].end(); ++itx)
				{
					Player * plr = (*itx);
					if(!plr) continue;
			...
			{
				for(PlayerSet::iterator itx = m_bg.m_players[0].begin(); itx != m_bg.m_players[0].end(); ++itx)
				{
					Player * plr = (*itx);
					if(!plr) continue;
...
void AlteracValley::OnCreate()
{
	...
	// Alliance Gate
	GameObject *gate = SpawnGameObject(AV_GAMEOBJECT_GATE, m_mapMgr->GetMapId(), 780.487f, -493.024f, 99.9553f, 3.0976f, 32, 114, 3.000000f);
	gate->SetFloatValue(GAMEOBJECT_ROTATION,0.0129570f);
	gate->SetFloatValue(GAMEOBJECT_ROTATION_01,-0.0602880f);
	gate->AddToWorld(m_mapMgr);
	m_gates.push_back(gate);

	// Horde gate
	gate = SpawnGameObject(AV_GAMEOBJECT_GATE, m_mapMgr->GetMapId(), -1375.73f, -538.966f, 55.3006f, 0.791198f, 32, 114, 3.000000f);
	...
	// generals/leaders!
	m_mapMgr->GetInterface()->SpawnCreature(AV_NPC_GENERAL_VANNDAR_STORMPIKE, 726.969604f, -9.716300f, 50.621391f, 3.377580f, true, false, 0, 0);
	m_mapMgr->GetInterface()->SpawnCreature(AV_NPC_GENERAL_DREK_THAR, -1367.080933f, -229.453140f, 98.421570f, 2.023553f, true, false, 0, 0);

	// some captains
	m_mapMgr->GetInterface()->SpawnCreature(AV_NPC_CAPTAIN_BALINDA_STONEHEARTH, -57.368469f, -286.770966f, 15.564562f, 6.068771f, true, false, 0, 0);
	m_mapMgr->GetInterface()->SpawnCreature(AV_NPC_CAPTAIN_GALVANGAR, -537.177429f, -168.004944f, 57.008938f, 2.749793f, true, false, 0, 0);
...
void AlteracValley::HookOnUnitKill(Player * plr, Unit * pVictim)
{
	...
	if(pVictim->GetEntry() == AV_NPC_GENERAL_VANNDAR_STORMPIKE) 
	{
		...
		for(PlayerSet::iterator itx = m_players[1].begin(); itx != m_players[1].end(); ++itx)
		{
			HonorHandler::AddHonorPointsToPlayer(*itx, 62);
		}
	}
	else if(pVictim->GetEntry() == AV_NPC_GENERAL_DREK_THAR)
	{
		...
		for(PlayerSet::iterator itx = m_players[0].begin(); itx != m_players[0].end(); ++itx)
		{
			HonorHandler::AddHonorPointsToPlayer(*itx, AV_HONOR_ON_KILL_BOSS);
		}
	}
...
void AlteracValley::Finish(uint32 losingTeam)
{
	...
	sEventMgr.AddEvent(dynamic_cast<CBattleground*>(this), &CBattleground::Close, EVENT_BATTLEGROUND_CLOSE, 120000, 1, EVENT_FLAG_DO_NOT_EXECUTE_IN_WORLD_CONTEXT);

	/* add the marks of honor to all players */
	SpellEntry * winner_spell = dbcSpell.LookupEntryForced(24955);
	SpellEntry * loser_spell = dbcSpell.LookupEntryForced(24954);
	for(uint32 i = 0; i < 2; ++i)
	{
		for(PlayerSet::iterator itr = m_players[i].begin(); itr != m_players[i].end(); ++itr)
		{
...
void AlteracValley::Herald(const char *format, ...)
{
	...
	va_list ap;
	va_start(ap, format);
	vsnprintf_s(msgbuf, sizeof(msgbuf), 128, format, ap);
	va_end(ap);
