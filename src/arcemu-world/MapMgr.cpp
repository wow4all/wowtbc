...
MapMgr::MapMgr(Map *map, uint32 mapId, uint32 instanceid) : CellHandler<MapCell>(map), _mapId(mapId), eventHolder(instanceid)
{
	...
	m_battleground = 0;
		...
		//WorldStateMgr
	m_WorldStateManager.SetMapMgr(this);
...
void MapMgr::SendPacketToPlayers(int32 iZoneMask, int32 iFactionMask, WorldPacket & pData)
{
	...
	for(;itr !=  m_PlayerStorage.end();)
	{
		...
		if(iZoneMask != ZONE_MASK_ALL && p->GetZoneId() != static_cast<uint32>(iZoneMask))
			continue;

		if(iFactionMask != ZONE_MASK_ALL && p->GetTeam() != static_cast<PlayerTeam>(iZoneMask))
			continue;
...
void MapMgr::SendPvPCaptureMessage(int32 iZoneMask, uint32 ZoneId, const char * Format, ...)
{
	...
	va_list ap;
	va_start(ap,Format);
	vsnprintf_s(msgbuf, sizeof(msgbuf), 256, Format, ap);
	va_end(ap);

	WorldPacket data(SMSG_PVP_CAPTURE_STATE_MSG, 256);
	data << uint32(ZoneId);
...
void MapMgr::RemoveAuraFromPlayers(int32 iFactionMask, uint32 uAuraId)
{
	...
	for(; itr !=  m_PlayerStorage.end();)
	{
		...
		if(iFactionMask != FACTION_MASK_ALL && p->GetTeam() != static_cast<PlayerTeam>(iFactionMask))
			continue;
