...
void Player::ForceAreaUpdate()
{
	...
	if(at != NULL)
	{
		...
		if(at->ZoneId && m_zoneId != at->ZoneId)
		{
			m_zoneId = at->ZoneId;
		}
	}
...
void Player::ZoneUpdate(uint32 ZoneId)
{
	...
	if(m_session->HasGMPermissions() && m_zoneId != 0 && at != NULL)
	{
		sChatHandler.BlueSystemMessage(m_session, "[%sSystem%s] |rEntered zone: %s%s", MSG_COLOR_WHITE, MSG_COLOR_LIGHTBLUE, MSG_COLOR_SUBWHITE, at->name);
	}
                    ...
                    if(GetZoneId() == 14 || GetZoneId() == 12)
                     {
                            ...
                            DuelingWith->BroadcastMessage("Cooldowns, health and mana Reset for %s ", DuelingWith->GetName());
                            ResetAllCooldowns();
                            DuelingWith->ResetAllCooldowns();
                            BroadcastMessage("Cooldowns, health and mana Reset for %s ", GetName());
