...
void SetGrave(MapMgr* pmgr)
{
	...
	if( ZMCityOwners == 0 )
	{
		pmgr->GetWorldStateManager()->SetWorldState(WORLDSTATE_ZANGARMARSH_CITY_ALLIANCE, 1);
		pmgr->GetWorldStateManager()->SetWorldState(WORLDSTATE_ZANGARMARSH_CITY_HORDE, 0);			
		pmgr->GetWorldStateManager()->SetWorldState(WORLDSTATE_ZANGARMARSH_CITY_NEUTRAL, 0);
	}
	else if( ZMCityOwners == 1 )
	{
		pmgr->GetWorldStateManager()->SetWorldState(WORLDSTATE_ZANGARMARSH_CITY_HORDE, 1);
		pmgr->GetWorldStateManager()->SetWorldState(WORLDSTATE_ZANGARMARSH_CITY_ALLIANCE, 0);
		pmgr->GetWorldStateManager()->SetWorldState(WORLDSTATE_ZANGARMARSH_CITY_NEUTRAL, 0);
	}
	else
	{
...
ARCEMU_INLINE void UpdateTowerCountZM(MapMgr* mgr)
{
	...
	{
		mgr->GetWorldStateManager()->SetWorldState(WORLDSTATE_ZANGARMARSH_SCOUT_NOT_READY_ALLIANCE, 1);
		mgr->GetWorldStateManager()->SetWorldState(WORLDSTATE_ZANGARMARSH_SCOUT_READY_ALLIANCE, 0);
		ZMg_superiorTeam = -1;
	}

	if( ZMg_superiorTeam == 1 && ZMg_hordeTowers != TOWER_COUNT )
	{
		mgr->GetWorldStateManager()->SetWorldState(WORLDSTATE_ZANGARMARSH_SCOUT_NOT_READY_HORDE, 1);
		mgr->GetWorldStateManager()->SetWorldState(WORLDSTATE_ZANGARMARSH_SCOUT_READY_HORDE, 0);
		ZMg_superiorTeam = -1;
	}
	...
	{
		ZMg_superiorTeam = 0;
		mgr->GetWorldStateManager()->SetWorldState(WORLDSTATE_ZANGARMARSH_SCOUT_NOT_READY_ALLIANCE, 0);
		mgr->GetWorldStateManager()->SetWorldState(WORLDSTATE_ZANGARMARSH_SCOUT_READY_ALLIANCE, 1);
	}

	if( ZMg_hordeTowers == TOWER_COUNT && ZMg_superiorTeam != 1 )
	{
		ZMg_superiorTeam = 1;
		mgr->GetWorldStateManager()->SetWorldState(WORLDSTATE_ZANGARMARSH_SCOUT_NOT_READY_HORDE, 0);
		mgr->GetWorldStateManager()->SetWorldState(WORLDSTATE_ZANGARMARSH_SCOUT_READY_HORDE, 1);
	}
}
...
public:
	...
	void AIUpdate()
	{
		...
		if( Status == 100 && m_bannerStatus != BANNER_STATUS_ALLIANCE )
		{
			...
			// state update
			_gameobject->GetMapMgr()->GetWorldStateManager()->SetWorldState(g_neutralStateFields_MAP[towerid], 0);
			_gameobject->GetMapMgr()->GetWorldStateManager()->SetWorldState(g_allianceStateFields_MAP[towerid], 1);
			
			_gameobject->GetMapMgr()->GetWorldStateManager()->SetWorldState(g_neutralStateFields_UI[towerid], 0);
			_gameobject->GetMapMgr()->GetWorldStateManager()->SetWorldState(g_allianceStateFields_UI[towerid], 1);
		...
		else if( Status == 0 && m_bannerStatus != BANNER_STATUS_HORDE )
		{
			...
			// state update
			_gameobject->GetMapMgr()->GetWorldStateManager()->SetWorldState(g_neutralStateFields_MAP[towerid], 0);
			_gameobject->GetMapMgr()->GetWorldStateManager()->SetWorldState(g_hordeStateFields_MAP[towerid], 1);

			_gameobject->GetMapMgr()->GetWorldStateManager()->SetWorldState(g_neutralStateFields_UI[towerid], 0);
			_gameobject->GetMapMgr()->GetWorldStateManager()->SetWorldState(g_hordeStateFields_UI[towerid], 1);
		...
		else if( m_bannerStatus != BANNER_STATUS_NEUTRAL )
		{
			...
			if( m_bannerStatus == BANNER_STATUS_ALLIANCE && Status <= 50 )
			{
				...
				// state update
				_gameobject->GetMapMgr()->GetWorldStateManager()->SetWorldState(g_allianceStateFields_MAP[towerid], 0);
				_gameobject->GetMapMgr()->GetWorldStateManager()->SetWorldState(g_neutralStateFields_MAP[towerid], 1);

				_gameobject->GetMapMgr()->GetWorldStateManager()->SetWorldState(g_allianceStateFields_UI[towerid], 0);
				_gameobject->GetMapMgr()->GetWorldStateManager()->SetWorldState(g_neutralStateFields_UI[towerid], 1);
			...
			else if( m_bannerStatus == BANNER_STATUS_HORDE && Status >= 50 )
			{
				...
				// state update
				_gameobject->GetMapMgr()->GetWorldStateManager()->SetWorldState(g_hordeStateFields_MAP[towerid], 0);
				_gameobject->GetMapMgr()->GetWorldStateManager()->SetWorldState(g_neutralStateFields_MAP[towerid], 1);

				_gameobject->GetMapMgr()->GetWorldStateManager()->SetWorldState(g_hordeStateFields_UI[towerid], 0);
				_gameobject->GetMapMgr()->GetWorldStateManager()->SetWorldState(g_neutralStateFields_UI[towerid], 1);
