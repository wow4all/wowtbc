...
void GatheredSilithyst(MapMgr * mgr)
{
	if(!mgr)
		return;
	if(mgr->GetMapId() == 1 ) // Kalimdor
	{
		mgr->GetWorldStateManager()->SetWorldState(WORLDSTATE_SILITHUS_SILITHYST_MAX, SILITHYST_MAX);
		mgr->GetWorldStateManager()->SetWorldState(WORLDSTATE_SILITHUS_GATHERED_ALLIANCE, 0);			
		mgr->GetWorldStateManager()->SetWorldState(WORLDSTATE_SILITHUS_GATHERED_HORDE, 0);
