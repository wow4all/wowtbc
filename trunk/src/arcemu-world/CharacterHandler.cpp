...
void WorldSession::FullLogin(Player * plr)
{
	...
	*/
	...
	{
		_player->BroadcastMessage("Tbc-Online Players: %s%u |rPeak: %s%u|r Accepted Connections: %s%u",
...
bool ChatHandler::HandleRenameCommand(const char * args, WorldSession * m_session)
{
	...
	{
		RedSystemMessage(m_session, "There is a player with that name already on fatal vengeance.");
		return true;
	}
