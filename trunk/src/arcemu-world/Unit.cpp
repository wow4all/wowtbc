...
bool CombatStatusHandler::InternalIsInCombat()
{
    ...
    {
        if(m_Unit->GetAIInterface()->GetNextTarget())
            return true;

        m_Unit->GetAIInterface()->LockAITargets(true);
        if(m_Unit->GetAIInterface()->getAITargetsCount())
        {
            m_Unit->GetAIInterface()->LockAITargets(false);
            return true;
		}
